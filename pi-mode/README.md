...
| **plan** | `extends: normal` + `write`/`edit` denied except `**/*.md`; bash classify overlay denies everything except `READONLY`. `internal: true` — reserved for a future `/plan` command. |
